...
export const SinglePiupiu = () => {
  ...
  const {} = useQuery({
    ...
    queryFn: async () => {
      ...
      const responseLiked = await likedRequest(
        id as string,
        storageUser.handle
      );
      const likedByUser: boolean = responseLiked.data.users.find( //determina se o usuário curtiu o piu
        (handle: string) => handle === storageUser.handle
      );
